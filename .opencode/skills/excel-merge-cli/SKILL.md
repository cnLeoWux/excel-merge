...
### Step 0: 智能意图识别与参数确认 (Smart Intent & Parameter Validation)
用户可能不会严格使用标准术语（如“合并”、“标注账期”），你需要具备高度的**上下文与模糊语义理解能力**。在开始执行前，按以下逻辑判断和沟通：

1. **隐式意图识别（看破不说破）**：
   - 如果用户**只上传了两个文件**（无说明，或只说“帮我弄下”、“这两个文件”）：
     请自动识别文件名，如果一个是订单（含“订单”），另一个是支付（含“流水/账单/明细”），**直接推断**其意图为匹配，并主动询问：“已收到订单和支付流水文件，是要帮您进行匹配合并吗？（如果还需要标注销售报表账期，请告诉我对应的月份，比如‘上个月’或‘3月’）”
2. **检查文件齐备度**：
   - 流程严格需要 **两个** 文件。如果用户只发了一个，或表达了对账意图但没发文件，主动提示：“我还缺少对应的文件（需要同时提供订单数据和支付流水），请您补充上传。”
3. **自然语言日期解析 (target_month)**：
   - 用户可能会用口语表达月份（如“三月的”、“上个月报表”、“2026年3月”）。
   - 你需要**自动将自然语言转换为 `YYYYMM` 格式**。如果年份未说明，结合当前现实时间推断。
   - 只要用户提到“做报表”、“账期”、“某月数据”等词汇，这就是要求生成报表。如果没识别到具体月份，再提问：“请问需要处理哪个月份的报表？（比如‘3月’或‘202603’）”
4. **不盲目行动**：
   - 在【两个文件就绪】且【用户意图确认（无论是主动说还是你询问后肯定）】之前，**不要**启动下载和执行命令。
