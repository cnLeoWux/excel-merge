...
**类型强制转换**：
- 列名包含 `"订单"` 或 `"流水"` 的列 → `astype(str).str.strip('="\\t ')` 清理多余字符
- 防止 pandas 将长数字订单号解析为浮点数
- 副作用：NaN 值会被转为字面量字符串 `"nan"`

**容错机制**：
- `on_bad_lines="warn"` 遇到格式错误的行会发出警告，防止数据静默丢失
...
读取时全局添加 `dtype=str` 防止科学计数法导致的数据截断；读取后对包含 `"订单"` 或 `"流水"` 关键字的列执行 `astype(str).str.strip('="\\t ')`，彻底剥离第三方支付平台为了防止 Excel 科学计数法而刻意添加的转义字符（如 `="123"` 或制表符）。
